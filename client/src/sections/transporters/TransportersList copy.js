...
import { setTransporter } from '../../redux/slices/transporter';
...
export default function TransportersList({
  ...
  const openCardHandler = (departure) => {
    console.log('🚀 ~ selectedTransporter:', departure);
  ...
  //   try {
  //     dispatch(getTransporters(data, activePage));

  //     // navigate(
  //     //   `/transporters/map?departure=${data?.departure}&destination=${data?.destination}&date=${data.date}`
  //     // );
  //   } catch (error) {
  //     console.error(error);
  //     // reset();
      ...
      <RootStyle>
        <ContentStyle>
          ...
          <CardContent>
            ...
            >
              {/* <InfiniteScroll
                dataLength={transporters.length}
                hasMore={transporters.length < totalTransporters}
